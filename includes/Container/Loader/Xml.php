...
namespace StorePlugin\CustomOrderStatus\Container\Loader;
...
class Xml {
	private function getComponent(\SimpleXmlElement $element, $forceInstance = false) {
		if ($forceInstance) return [Dice::INSTANCE => (string) $element];
		else if ($element->instance) return [Dice::INSTANCE => (string) $element->instance];
		else return (string) $element;
	}

	private function loadV1(\SimpleXmlElement $xml, Dice $dice) {
		$rules = [];
	...
	private function loadV2(\SimpleXmlElement $xml, Dice $dice) {
		$rules = [];
	...
	public function load($xml, Dice $dice = null, $displayWarning = true) {

		if ($displayWarning) {
			trigger_error('Deprecated: The XML loader is being removed in the next version of Dice please use $xmlLoader->convert(\'' . esc_html($xml) . '\', \'path/to/rules.json\'); to convert the rules to JSON format', E_USER_WARNING);
		}
